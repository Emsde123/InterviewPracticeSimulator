import QuestionsDB.SQLQuestions;

import java.util.ArrayList;
import java.util.Scanner;

public class MySQLQuiz implements MyQuiz{
    ...
    public int getRandomNumber() {
        int max = SQLQuestions.sqlQuestions.size() - 1;
        int rand = (int) (Math.random() * (max));
        if(arr.contains(rand)){

            do{
                rand = (int) (Math.random() * max);
            }while(arr.contains(rand) && arr.size() != SQLQuestions.sqlQuestions.size());
    ...
    public void generateQuestion() {
        System.out.println(SQLQuestions.sqlQuestions.get(getRandomNumber()));
        ++totalAsked;
    }
    ...
    @Override
    public void start() {
        System.out.println(ConsoleColors.ANSI_BLUE + "Welcome! SQL Quiz is starting now.." + ConsoleColors.ANSI_RESET);
        int count = 0;
        ...
        do{
            ...
            if(answer.equalsIgnoreCase("y")){
                ...
                wrongQuestions.add(SQLQuestions.sqlQuestions.get(random));
        ...
        }while(count < SQLQuestions.sqlQuestions.size());
    ...
    public void generateAnswer() {
        System.out.println(SQLQuestions.sqlAnswers.get(random));
    }
}
