import QuestionsDB.APIQuestions;

import java.util.ArrayList;
import java.util.Scanner;

public class MyCombinedAPIQuiz implements MyQuiz{
    ...
    public int getRandomNumber() {
        ...
        int max = APIQuestions.combinedAPIQuestions.size() - 1;
        int rand = (int) (Math.random() * (max));
        if(arr.contains(rand)){
            do{
                rand = (int) (Math.random() * max);
            }while(arr.contains(rand) && arr.size() != APIQuestions.combinedAPIQuestions.size());
    ...
    public void generateQuestion() {
        System.out.println(APIQuestions.combinedAPIQuestions.get(getRandomNumber()));
        ++totalAsked;
    }
    ...
    @Override
    public void start() {
        System.out.println(ConsoleColors.ANSI_BLUE + "Welcome! Combined API quiz is starting now.." + ConsoleColors.ANSI_RESET);
        int count = 0;
        ...
        do{
            ...
            if(answer.equalsIgnoreCase("y")){
                ...
                wrongQuestions.add(APIQuestions.combinedAPIQuestions.get(random));
        ...
        }while(count < APIQuestions.combinedAPIQuestions.size());
    ...
    public void generateAnswer() {
        System.out.println(APIQuestions.combinedAPIAnswers.get(random));
    }
}
