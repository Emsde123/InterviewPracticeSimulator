import QuestionsDB.MavenQuestions;

import java.util.ArrayList;
import java.util.Scanner;

public class MyMavenQuiz implements MyQuiz {
    ...
    public int getRandomNumber() {
        int max = MavenQuestions.mavenGITJenkinsQuestions.size() - 1;
        int rand = (int) (Math.random() * (max));
        if(arr.contains(rand)){

            do{
                rand = (int) (Math.random() * max);
            }while(arr.contains(rand) && arr.size() != MavenQuestions.mavenGITJenkinsQuestions.size());
    ...
    public void generateQuestion() {
        System.out.println(MavenQuestions.mavenGITJenkinsQuestions.get(getRandomNumber()));
        ++totalAsked;
    }
    ...
    @Override
    public void start() {
        System.out.println(ConsoleColors.ANSI_BLUE + "Welcome! Maven/GIT Quiz is starting now.." + ConsoleColors.ANSI_RESET);
        int count = 0;
        ...
        do{
            ...
            if(answer.equalsIgnoreCase("y")){
                ...
                wrongQuestions.add(MavenQuestions.mavenGITJenkinsQuestions.get(random));
            }
        ...
        }while(count < MavenQuestions.mavenGITJenkinsQuestions.size());
    ...
    public void generateAnswer() {
        System.out.println(MavenQuestions.mavenGITJenkinsAnswers.get(random));
    }
}
