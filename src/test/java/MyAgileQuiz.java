import QuestionsDB.AgileQuestions;

import java.util.ArrayList;
import java.util.Scanner;

public class MyAgileQuiz implements MyQuiz {
   ...
   private final ArrayList<Integer> arr = new ArrayList<>();
        ...
        private int random;
        AgileQuestions AQ = new AgileQuestions();
    MyAgileQuiz(){
    ...
    public int getRandomNumber() {
        ...
        int max = AgileQuestions.agileQuestions.size() - 1;
        int rand = (int) (Math.random() * (max));
        if(arr.contains(rand)){
            do{
                rand = (int) (Math.random() * max);
            }while(arr.contains(rand) && arr.size() != AgileQuestions.agileQuestions.size());
    ...
    public void generateQuestion() {
        System.out.println(AgileQuestions.agileQuestions.get(getRandomNumber()));
        ++totalAsked;
    }
    ...
    @Override
    public void start() {
        System.out.println(ConsoleColors.ANSI_BLUE + "Welcome! SDLC Quiz is starting now.." + ConsoleColors.ANSI_RESET);
        int count = 0;
        ...
        do{
            ...
            if(answer.equalsIgnoreCase("y")){
                ...
                wrongQuestions.add(AgileQuestions.agileQuestions.get(random));
        ...
        }while(count < AgileQuestions.agileQuestions.size());
    ...
    public void generateAnswer() {
        System.out.println(AgileQuestions.agileAnswers.get(random));
    }
}
